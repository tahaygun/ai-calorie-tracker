...
export default function SettingsPrompt({ type, onOpenSettings }: SettingsPromptProps) {
  ...
  const linkColor = isApiKey ? 'text-yellow-400 hover:text-yellow-300' : 'text-blue-400 hover:text-blue-300';
  ...
  return (
    <div className={`mb-4 p-3 ${bgColor} border ${borderColor} rounded`}>
      <p className={`text-sm ${textColor}`}>
        {message}{' '}
        <button onClick={onOpenSettings} className={`${linkColor} underline`}>
          Open Settings
        </button>
      </p>
    </div>
